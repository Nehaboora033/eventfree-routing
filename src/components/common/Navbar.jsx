...
const Navbar = () => {
  ...
  return (
    <>
      <nav className={`fixed top-0 z-20 w-full  ${scrolled ? 'shadow-card' : ''}`}>
          ...
          <div className='flex items-center gap-14'>
            <div className='flex gap-16 '>
              {NAV_LINKS.map((item, index) => (
                <NavLink to={item.link == "home" ? '/' : `/${item.link}`} key={index} className={({ isActive }) => `font-normal text-base uppercase ${isActive ? 'opacity-100' : 'opacity-70'}`}>
                  {item.name}
                </NavLink>
              ))}
            </div>
